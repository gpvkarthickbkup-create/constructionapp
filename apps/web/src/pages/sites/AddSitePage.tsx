...
export default function AddSitePage() {
  ...
  const handleSiteNameChange = (value: string) => {
    ...
    if (!form.siteCode || form.siteCode === generateCode(form.siteName)) {
      updateField('siteCode', generateCode(value));
    }
  };
  ...
  const handleCreateClient = async () => {
    if (!newClient.name.trim() || !newClient.mobile.trim()) {
      toast.error(t('common.requiredFields', 'Name and mobile are required'));
      return;
    }
    setCreatingClient(true);
    try {
      const res = await api.post('/customers', newClient);
      const created = res.data.data || res.data;
      toast.success(t('customers.addSuccess', 'Client added'));
      setClients(prev => [created, ...prev]);
      selectClient(created);
      setShowNewClientForm(false);
      setNewClient({ name: '', mobile: '', address: '' });
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.clientName.trim() && !selectedClientId) {
      toast.error('Client name is required. Please select or add a client.');
      return;
    }
    ...
    try {
      const body: Record<string, any> = {
        ...
        totalSqft: sqft || undefined,
        customerRatePerSqft: customerRate || undefined,
        builderRatePerSqft: builderRate || undefined,
        customerEstimate: customerEstimate || undefined,
        builderEstimate: builderEstimate || undefined,
        saleAmount: saleAmount || undefined,
        estimatedProfit: estimatedProfit || undefined,
        estimatedBudget: builderEstimate || undefined,
        notes: form.notes.trim() || undefined,
      };

      await api.post('/sites', body);
      toast.success(t('sites.siteCreated', 'Site created successfully'));
      navigate('/app/sites');
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{t('sites.addSite', 'Add Site')}</h1>
          <p className="text-sm text-muted-foreground">{t('sites.addSiteDesc', 'Create a new construction site')}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="overflow-hidden shadow-md border-0">
          <div className="bg-gradient-to-r from-[#f26f31] to-[#c9531a] px-6 py-4">
            <h2 className="text-white font-semibold flex items-center gap-2">
              ...
              {t('sites.selectClient', 'Select Client')} <span className="text-white/80">*</span>
            </h2>
          </div>
          <CardContent className="p-6 space-y-4">
            ...
            {selectedClientId && (
              <div className="flex items-center gap-3 rounded-xl bg-green-50 border border-green-200 p-3">
                <div
                  className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl text-white font-bold text-lg shadow-md"
                  style={{ backgroundColor: getAvatarColor(form.clientName) }}
                >
                  {(form.clientName || 'C').charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-base">{form.clientName}</p>
                  <p className="text-sm text-muted-foreground flex items-center gap-1">
                    <Phone className="h-3.5 w-3.5" /> {form.clientMobile || '-'}
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => { setSelectedClientId(''); updateField('clientName', ''); updateField('clientMobile', ''); }}
              ...
              <>
                {clients.length > 0 && (
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t('common.search') + ' ' + t('nav.clients', 'clients') + '...'}
                ...
                {clients.length === 0 && (
                  <p className="text-center text-sm text-muted-foreground py-4">
                    {t('customers.emptyTitle', 'No clients yet')}
                  </p>
                )}
                ...
                <Button
                  ...
                  className="w-full rounded-xl border-dashed border-[#f26f31] text-[#f26f31] hover:bg-[#f26f31]/5"
                >
                  <Plus className="mr-2 h-4 w-4" />
                  {t('customers.addCustomer', 'New Client')}
                </Button>
              </>
            )}
            ...
            {!selectedClientId && showNewClientForm && (
              <div className="space-y-3 rounded-xl border border-dashed border-[#f26f31]/40 p-4 bg-orange-50/30">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    className="bg-[#f26f31] hover:bg-[#c9531a] rounded-xl"
                  >
                    {creatingClient ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Plus className="mr-1 h-4 w-4" />}
                    {t('common.save', 'Save')}
                  </Button>
                  <Button
                    type="button"
                    variant="ghost"
                    onClick={() => { setShowNewClientForm(false); setNewClient({ name: '', mobile: '', address: '' }); }}
                    className="rounded-xl"
                  >
          ...
          <CardContent className="p-6 space-y-5">
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={form.siteCode}
                  onChange={(e) => updateField('siteCode', e.target.value.toUpperCase())}
                  placeholder={t('sites.siteCodePlaceholder', 'Auto-generated')}
              ...
              {/* Client Name (auto-filled but editable) */}
              <div className="space-y-1.5">
                <label className="text-sm font-medium flex items-center gap-1.5">
        ...
        <Card className="overflow-hidden shadow-md border-0">
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <div className="border-t pt-6 space-y-3">
              ...
              <p className="text-xs text-muted-foreground">Optional — enter after sale is confirmed. Until then, Customer Estimate (SQFT × Rate) is used for margin calculation.</p>
              <Input
                type="number"
              ...
              {/* Variance from customer estimate */}
        ...
        <div className="flex items-center justify-end gap-3 pb-6">
          ...
          >
            ...
            {t('sites.createSite', 'Create Site')}
          </Button>
        </div>
      </form>
    </div>
